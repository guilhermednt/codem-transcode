...
var Job = JobUtils.getDatabase().define('Job', {
  ...
  lastNotifiedProgress:    { type: Sequelize.FLOAT, defaultValue: 0.0 },
  ...
  instanceMethods: {
    ...
    progressHandler: function(data) {
      ...
      var lastProgress = this.lastNotifiedProgress;

      (isNaN(this.duration) || this.duration == 0) ? this.extractDuration(data.toString()) : this.extractProgress(data.toString());

      var delta = Math.abs(this.progress - lastProgress);
      if (delta >= 0.05 || this.progress >= 1) {
